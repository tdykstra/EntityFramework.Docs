...
| -c | `--context <DBCONTEXT>`           | The `DbContext` class to use. Class name only or fully qualified with namespaces.  If there is only one DbContext in the project, EF Core will find it, and you can omit this option. |
...
| -c              | `--context <NAME>`                       | The name of the `DbContext` class.                                                                       |
...
| -t              | `--table <TABLE_NAME>`...                | The tables to generate entity types for. To specify multiple tables, repeat the `-t` or `--table` for each one.                                                          |
